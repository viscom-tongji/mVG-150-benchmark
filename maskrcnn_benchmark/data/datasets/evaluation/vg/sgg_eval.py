...
"""
Semantic Recall:
Evaluates recall using semantic similarity for predicate matching.
Based on No Graph Constraint Recall but uses semantic similarity instead of exact matching.
"""
class SGSemanticRecall(SceneGraphEvaluation):
    ...
    def _get_similarity_vectorized(self, pred_labels, gt_label):
        """
        Get semantic similarity between multiple predictions and one GT predicate.
        Vectorized version for efficiency.
        Args:
            pred_labels: np.array of predicate labels (1-based)
            gt_label: int, GT predicate label (1-based)
        Returns:
            np.array of similarities
        """
        ...
        # Handle boundary for predictions
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        # NG-Recall style: compute scores without graph constraint
        obj_scores_per_rel = obj_scores[pred_rel_inds].prod(1)
        nogc_overall_scores = obj_scores_per_rel[:, None] * rel_scores[:, 1:]
        
        # Get top-100 predictions globally (sorted by score)
        nogc_score_inds = argsort_desc(nogc_overall_scores)[:100]
        
        # Build prediction info for top-100
        nogc_pred_rels = np.column_stack((
            pred_rel_inds[nogc_score_inds[:, 0]],
            nogc_score_inds[:, 1] + 1
        ))
        
        # Build ground truth triplets and boxes
        gt_triplets, gt_triplet_boxes, _ = _triplet(gt_rels, gt_classes, gt_boxes)
        local_container['gt_triplets'] = gt_triplets
        local_container['gt_triplet_boxes'] = gt_triplet_boxes
        
        # Build prediction triplets and boxes for top-100 (vectorized)
        pred_triplets = np.column_stack((
            pred_classes[nogc_pred_rels[:, 0]],
            nogc_pred_rels[:, 2],
            pred_classes[nogc_pred_rels[:, 1]]
        ))
        pred_sub_boxes = pred_boxes[nogc_pred_rels[:, 0]]  # (100, 4)
        pred_obj_boxes = pred_boxes[nogc_pred_rels[:, 1]]  # (100, 4)
        
        gt_sub_boxes = gt_triplet_boxes[:, :4]  # (num_gt, 4)
        gt_obj_boxes = gt_triplet_boxes[:, 4:]  # (num_gt, 4)
        
        # Pre-compute all IoUs at once (vectorized)
        # sub_ious: (num_gt, 100), obj_ious: (num_gt, 100)
        sub_ious = bbox_overlaps(gt_sub_boxes, pred_sub_boxes[:100])
        obj_ious = bbox_overlaps(gt_obj_boxes, pred_obj_boxes[:100])
        
        # Pre-compute class matches (vectorized)
        # gt_triplets[:, 0] is gt_sub_class, pred_triplets[:, 0] is pred_sub_class
        sub_class_match = (gt_triplets[:, 0:1] == pred_triplets[:100, 0:1].T)  # (num_gt, 100)
        obj_class_match = (gt_triplets[:, 2:3] == pred_triplets[:100, 2:3].T)  # (num_gt, 100)
        
        # Combined structural match mask
        struct_match = sub_class_match & obj_class_match & (sub_ious >= iou_thres) & (obj_ious >= iou_thres)
        
        # For each K, compute semantic recall
        for k in self.result_dict[mode + '_semantic_recall']:
            k_struct_match = struct_match[:, :k]  # (num_gt, k)
            k_pred_labels = pred_triplets[:k, 1]  # (k,)
...
"""
Mean Semantic Recall:
Calculates mean semantic recall across predicate classes.
Combines Mean Recall per-predicate logic with semantic similarity scoring.
"""
class SGMeanSemanticRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        # Collect semantic similarity scores per predicate class
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        ...
        # Get top-100 predictions globally
        nogc_score_inds = argsort_desc(nogc_overall_scores)[:100]
        ...
        # Build prediction triplets and boxes (vectorized)
        pred_triplets = np.column_stack((
            pred_classes[nogc_pred_rels[:, 0]],
            nogc_pred_rels[:, 2],
            pred_classes[nogc_pred_rels[:, 1]]
        ...
        # For each K, collect semantic recall per predicate class
        for k in self.result_dict[mode + '_mean_semantic_recall_collect']:
            k_struct_match = struct_match[:, :k]  # (num_gt, k)
            k_pred_labels = pred_triplets[:k, 1]  # (k,)
            
            # Collect similarity scores per GT predicate class
            ...
            for gt_idx in range(num_gt):
                ...
                # Store max similarity for this GT's predicate class
                if int(gt_pred_label) < self.num_rel:
                    similarity_per_class[int(gt_pred_label)].append(max_similarity)
            
            # For each predicate class, calculate mean similarity and store
            for n in range(self.num_rel):
...
"""
Diversity-Aware Recall (DA-R):
Evaluates recall with weighted scoring based on predicate categories.
...
class SGCategoryWeightedRecall(SceneGraphEvaluation):
    ...
    def __init__(self, result_dict, ind_to_predicates, print_detail=False):
        ...
        # Build predicate name to index mapping (1-based, 0 is background)
        self.predicate_to_idx = {name.lower(): idx for idx, name in enumerate(ind_to_predicates)}
        
        # Build predicate index to category mapping
    ...
    def _get_predicate_category(self, pred_idx):
        """Get category index for a predicate, returns -1 if not in any category"""
        return self.pred_idx_to_category.get(pred_idx, -1)

    def _aggregate_gt_object_pairs(self, gt_rels, gt_boxes):
        """
        Aggregate GT predicates by object pair (same bbox coordinates).
        Returns:
            aggregated_pairs: list of dicts, each containing:
                - sub_idx: subject index in gt_boxes
                - obj_idx: object index in gt_boxes
                - predicates: list of predicate indices
                - categories: set of category indices covered
                - weight: computed weight for this pair
        """
        # Group by (sub_box_tuple, obj_box_tuple)
        pair_to_predicates = {}
        for rel in gt_rels:
        ...
        # Build aggregated pairs with weights
        aggregated_pairs = []
        for (sub_box, obj_box, sub_idx, obj_idx), predicates in pair_to_predicates.items():
            ...
            # Weight = num_categories + (num_predicates - num_categories) * 0.5
            ...
            aggregated_pairs.append({
                ...
                'weight': weight,
                'hit_categories': set(),  # Track which categories have been hit
                'hit_score': 0.0  # Accumulated hit score
            })
    ...
    def calculate_recall(self, global_container, local_container, mode):
        """
        Calculate category weighted recall with efficient matching:
        1. First filter by (sub_cls, obj_cls, relation) triple to reduce candidates
        2. Then compute IoU only for filtered candidates
        3. Score hits: 1 for first hit in category, 0.5 for subsequent
        """
        obj_scores = local_container['obj_scores']
        pred_rel_inds = local_container['pred_rel_inds']
        rel_scores = local_container['rel_scores']
        ...
        # Get top predictions (no graph constraint style)
        obj_scores_per_rel = obj_scores[pred_rel_inds].prod(1)
        nogc_overall_scores = obj_scores_per_rel[:, None] * rel_scores[:, 1:]
        nogc_score_inds = argsort_desc(nogc_overall_scores)[:100]
        
        # Build prediction info: (pair_idx, pred_label, sub_idx, obj_idx)
        pred_info = []
        for i, (pair_idx, rel_idx) in enumerate(nogc_score_inds):
            sub_idx = pred_rel_inds[pair_idx, 0]
            obj_idx = pred_rel_inds[pair_idx, 1]
            pred_label = rel_idx + 1  # +1 because we skip background
            pred_info.append({
                'rank': i,
                'sub_idx': sub_idx,
                'obj_idx': obj_idx,
                ...
                'sub_box': pred_boxes[sub_idx],
                'obj_box': pred_boxes[obj_idx],
                'pred_label': pred_label
            })
        
        # Build GT triple index for efficient filtering: (sub_cls, obj_cls, pred_label) -> list of aggregated_gt indices
        gt_triple_index = {}  # (sub_cls, obj_cls, pred_label) -> list of aggregated_gt indices
        for gt_idx, gt_pair in enumerate(aggregated_gt):
            sub_cls = gt_classes[gt_pair['sub_idx']]
            obj_cls = gt_classes[gt_pair['obj_idx']]
            # For each predicate in this aggregated pair, create a triple key
        ...
        # Pre-compute GT boxes for aggregated pairs
        gt_sub_boxes = np.array([list(p['sub_box']) for p in aggregated_gt])
        gt_obj_boxes = np.array([list(p['obj_box']) for p in aggregated_gt])
        
        # For each K, calculate weighted recall
        for k in self.result_dict[mode + '_category_weighted_recall']:
            # Reset hit tracking for this K
            for gt_pair in aggregated_gt:
                gt_pair['hit_categories'] = set()
                gt_pair['hit_score'] = 0.0
                gt_pair['hit_predicates'] = set()  # Track which predicates have been matched to avoid duplicate scoring
            ...
            for pred in pred_info[:k]:
                ...
                # Step 1: Filter by (sub_cls, obj_cls, relation) triple (efficient O(1) lookup)
                if pred_triple_key not in gt_triple_index:
                    continue
                
                candidate_gt_indices = gt_triple_index[pred_triple_key]
                
                # Step 2: Compute IoU only for filtered candidates (predicate already matched)
                ...
                for gt_idx in candidate_gt_indices:
                    ...
                    # Step 3: Score the hit (predicate already matched via triple key)
                    # Deduplicate: each predicate can only be scored once per GT pair
                    if pred_label in gt_pair['hit_predicates']:
                        continue
                    gt_pair['hit_predicates'].add(pred_label)
                    
                    # Get category of this predicate
                    cat = self._get_predicate_category(pred_label)
                    
                    if cat >= 0 and cat in gt_pair['categories']:
                        # Check if this category was already hit
                        ...
                        # Predicate not in any category, count as 0.5
                        gt_pair['hit_score'] += 0.5
...
"""
Diversity-Aware Mean Recall (DA-mR@K):
Similar to Mean Recall, but averages across predicate categories instead of individual predicates.
For each category c that exists in the image's GT:
    Recall_c = (Top-K中正确预测且属于类别c的GT三元组数量) / (该图片中属于类别c的GT三元组总数)
DA-mR@K = average of Recall_c for all categories c present in the image
...
class SGNGCategoryWeightedMeanRecall(SceneGraphEvaluation):
    ...
    def __init__(self, result_dict, ind_to_predicates, print_detail=False):
        ...
        # Build predicate name to index mapping (case-insensitive)
        self.predicate_to_idx = {name.lower(): idx for idx, name in enumerate(ind_to_predicates)}
    ...
    def register_container(self, mode):
        # Final mean recall values
        self.result_dict[mode + '_ng_category_weighted_mean_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        # Per-image category recall collection: [k][category_idx] = list of recalls
        self.result_dict[mode + '_ng_cwmr_collect'] = {
            20: [[] for _ in range(self.num_categories)],
            50: [[] for _ in range(self.num_categories)],
            100: [[] for _ in range(self.num_categories)]
        }
        # Per-category recall list for final output
    ...
    def _aggregate_gt_object_pairs(self, gt_rels, gt_boxes):
        """
        Aggregate GT predicates by object pair (same as SGCategoryWeightedRecall).
        Returns aggregated pairs with per-category weight decomposition.
        """
        pair_to_predicates = {}
        for rel in gt_rels:
            sub_idx, obj_idx, pred_label = int(rel[0]), int(rel[1]), int(rel[2])
        ...
        aggregated_pairs = []
        for (sub_box, obj_box, sub_idx, obj_idx), predicates in pair_to_predicates.items():
            # Count predicates per category
            category_pred_count = {}  # cat_idx -> count of predicates in this category
            for pred in predicates:
                cat = self._get_predicate_category(pred)
                if cat >= 0:
                    category_pred_count[cat] = category_pred_count.get(cat, 0) + 1
            
            # Per-category weight: weight_c = 1.0 + (n_c - 1) * 0.5
            category_weights = {}
            ...
            aggregated_pairs.append({
                ...
                'obj_box': obj_box,
                'predicates': predicates,
                'category_pred_count': category_pred_count,
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        """
        Collect per-category DA-R@K recall for this image.
        Uses the same matching and scoring logic as SGCategoryWeightedRecall:
        1. Aggregate GT pairs
        2. Triple key filtering + IoU verification
        3. 1.0/0.5 weighted scoring
        4. Decompose scores per category
        """
        obj_scores = local_container['obj_scores']
        pred_rel_inds = local_container['pred_rel_inds']
        rel_scores = local_container['rel_scores']
        ...
        # Aggregate GT object pairs (same as DA-R@K)
        aggregated_gt = self._aggregate_gt_object_pairs(gt_rels, gt_boxes)
        
        # Check if any category has weight > 0
        has_any_category = any(len(p['category_weights']) > 0 for p in aggregated_gt)
        if not has_any_category:
            return
        
        # Get top predictions (no graph constraint style, same as DA-R@K)
        obj_scores_per_rel = obj_scores[pred_rel_inds].prod(1)
        nogc_overall_scores = obj_scores_per_rel[:, None] * rel_scores[:, 1:]
        nogc_score_inds = argsort_desc(nogc_overall_scores)[:100]
        ...
        # Build GT triple index for efficient filtering
        gt_triple_index = {}
        for gt_idx, gt_pair in enumerate(aggregated_gt):
            sub_cls = gt_classes[gt_pair['sub_idx']]
            obj_cls = gt_classes[gt_pair['obj_idx']]
        ...
        for k in self.result_dict[mode + '_ng_cwmr_collect']:
            ...
            for pred in pred_info[:k]:
                ...
                for gt_idx in candidate_gt_indices:
                    ...
                    # Deduplicate: each predicate scored once per GT pair
                    if pred_label in gt_pair['hit_predicates']:
                        continue
                    gt_pair['hit_predicates'].add(pred_label)
                    
                    # Score by category (same 1.0/0.5 rule as DA-R@K)
                    cat = self._get_predicate_category(pred_label)
                    ...
                    if cat >= 0 and cat in gt_pair['category_weights']:
                        ...
                        raise ValueError(
                            f"[NGCategoryWeightedMeanRecall] 预测谓词 '{pred_name}' (idx={pred_label}) "
                            f"不属于五大语义类别中的任何一个，请检查 PREDICATE_CATEGORIES 定义是否覆盖了所有谓词。"
                        )
            
            # Compute per-category recall for this image
            # For each category: recall_c = sum(min(hit_c, weight_c)) / sum(weight_c)
    ...
    def calculate_mean_recall(self, mode):
        """
        Calculate DA-mR@K by averaging per-category recalls across the dataset.
        """
        for k in self.result_dict[mode + '_ng_category_weighted_mean_recall']:
            ...
            # Average across all categories (including those with 0 recall if they exist in dataset)
